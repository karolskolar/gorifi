...
**Adaptation list (exhaustive — everything else is verbatim; A9 added by RD-DS-5, A10 by
RD-FL-8a):**
...
| A10 | *(not in theme.css)* | **added** at the end of the file: one rule listing `.field-lbl`, `.field-help`, `.sub`, `.appbar .chip`, `.badge`, `.banner`, `.copyrow .val`, `.tab`, `.tbl td`, `.statuspill` (each prefixed `:where(.app, .modal-layer) `) `{line-height:normal}` | **The second ADDITION, and the same defect as A9 one layer out.** A9 covered the two CONTROL classes; Tailwind preflight's `html{line-height:1.5}` reaches every *other* class the canon leaves silent too. theme.css carries exactly **seven** `line-height` declarations — `.appbar .titles` (33), `.appbar .titles .t` (34), `.h-screen` (44), `.stepper button` (95), `.cartbar .sum` (163), `.pimg .lbl` (186), `.modal .m-title` (193) — and **every class in this rule is absent from that list**, so the prototype computes the UA default `normal` for all of them. ⚠ A10 first shipped covering only the three plain-text classes and deferred the rest to "the module-03 audit"; that was wrong on its own terms, since **four of the residual classes are rendered by module 03's own shipped surface** and RD-FL-8b (the audit) carries no code — it would have re-baselined fidelity against inflated boxes. RD-FL-8a therefore measured and swept the whole set. **Method:** identical markup injected into the canon `.app` (prototype over HTTP) and into the built port's `.app`, fonts force-loaded on both sides (`normal` is font-metric driven, so an unswapped webfont silently changes the answer); border-box heights. **canon → port before → delta:** `.appbar .chip` 25 → 28.5 (+3.5); `.badge` 26 → 30 (+4); `.banner` 52 → 56.5 (+4.5); `.banner.slim` 40 → 44.25 (+4.25); `.copyrow .val` 42 → 46.75 (+4.75); `.tab` 42 → 45 (+3, the canon's `min-height:42px` stopped governing entirely); `.cat-tabs .tab` 40 → 41; `.tabgroup .tab` 40 → 41; `.tbl td` 46.5 → 50.5 (+4); `.statuspill` 36 → 40.25 (+4.25); and the three original plain-text classes `.field-lbl` 15 → 18.75, `.field-help` 15 → 19.5, `.sub` 15 → 19.5 @13px / 21 @14px. After the rule the port reproduces the canon **exactly on every one of these**. Every delta is a REDUCTION; nothing here can make a box taller. Knock-on: `.copyrow`'s copy `.btn` was being stretched to 46.75 by the inflated `.val` and returns to its canon 44. **Deliberately excluded, and the only one: `.tabbadge`** — canon 22, port 22, delta 0, because it declares `height:22px` with `display:inline-flex;align-items:center`, so the line box is centred inside a fixed box and the inherited 1.5 changes nothing measurable. Nothing else in theme.css renders text and is line-height-silent; the sweep behind this table walked every element on all 16 prototype screens. Scoped to an explicit class list rather than a blanket `line-height:normal` inside `.app`, on A9's rationale verbatim: a broad rule would also restyle the shadcn children still rendering inside a half-migrated `.app` (UC-DS-002) — the legacy login card and the credential-setup dialog are live examples. `:where()` keeps the (0,1,0) specificity so `.mono.sub`, `.badge.acc`, `.banner.slim` and every compound keeps its own cascade. Admin reach is nil **by construction**: `.app` is declared only in `FriendPortal.vue` and `.modal-layer` only in `NeoModal.vue`. |
